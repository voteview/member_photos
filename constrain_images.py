...
def constrain_folder(folder, override):
    ...
    if override:
        ...
        files_resized = ["%s.jpg" % x.replace("raw/", "").rsplit(".", 1)[0] for x in files]
        files_edit = [x for x, y in zip(files, files_resized) if not os.path.isfile(y) or os.path.getmtime(y) < os.path.getmtime(x)]
...
def constrain_image(file_name):
    ...
    with Image(filename=file_name) as img_in:
        ...
        new_filename = "%s.jpg" % file_name.replace("raw/", "").rsplit(".", 1)[0]

        if aspect_ratio > 0.75 and aspect_ratio < 0.85 or "bio_guide" in file_name:
            print("\t Aspect ratio OK, resizing.")
            args = ["mogrify", "-verbose", "-format jpg", "-quality 75", '-resize "600x600>"', "-path %s" % new_folder, file_name]
        else:
            ...
            if new_height > 600:
                new_height = 600
                new_width = 480
            print("\t Rescaling image from %s x %s (AR %s) to %s x %s" % (width, height, aspect_ratio, new_width, new_height))
            args = ["smartcrop", "--width %s" % new_width, "--height %s" % new_height, "--faceDetection", "--outputFormat jpg", "--quality 80", file_name, new_filename]
