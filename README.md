...
* `config/database-raw.json`: Large raw database dump, used for flat-file searches. Generated by `config/dump_db.csv`

### Behind the Scenes:

* `config/dump_db.py`: Dumps current Mongo database to flatfile (use this first, to update the local flat file). Requires our local MongoDB instance.
* `config/dump_csv.py`: Dumps the current images to a `members.csv` file. Can take `--type flat` to dump from flat file. (Use this after updates to correctly log the information in the menbers file)
