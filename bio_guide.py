...
def single_download(type, icpsr):
	""" Download a single ICPSR. """

	if type == "flat":
		people = json.load(open("config/database-raw.json", "r"))
		bioguide_id = next(x["bioguide_id"] for x in people if x["icpsr"] == icpsr and "bioguide_id" in x)
		if bioguide_id:
		...
		result = db.voteview_members.find_one(query, {"bioguide_id": 1, "_id": 0})
		if result:
