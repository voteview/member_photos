...
def get_provenance(padded_icpsr):
	...
	global CONFIG
	if not CONFIG or not "provenance" in CONFIG:
		CONFIG["provenance"] = json.load(open("provenance.json", "r"))

	for k, v in CONFIG["provenance"].iteritems():
		if padded_icpsr in v:
			return k
...
def do_dump(type):
	...
	csv_rows = []
	bio_images = [x.rsplit("/", 1)[1].rsplit(".", 1)[0] for x in glob.glob("../images/bio_guide/*.jpg")]
	wiki_images = [x.rsplit("/", 1)[1].rsplit(".", 1)[0] for x in glob.glob("../images/wiki/*.jpg")]
	manual_images = [x.rsplit("/", 1)[1].rsplit(".", 1)[0] for x in glob.glob("../images/manual/*.jpg")]
	...
	# Do CSV write
	field_names = ["name", "icpsr", "state", "party", "congress", "chamber", "born", "died", "image", "source", "provenance"]
	with open("../members.csv", "w") as f:
		out_dump = csv.DictWriter(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC, fieldnames=field_names)
